            ...
            checks.forEach(check => {
                ...
                _data.read('checks', check, (err, originalCheckData) => {
                    if (!err && originalCheckData) {
                        ...
                        log('Error: Could not read one of the checks data');
                    }
                });
            });
        } else {
            log("Error : Could not find any checks to process");
        }
...
workers.validateCheckData = (originalCheckData) => {
    ...
    if (originalCheckData.id &&
        ...
        log('Error : one of the checks is not properly formatted. Skipping it.');
    }
};
...
workers.alertUserToStatusChange = (newCheckData) => {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, (err) => {
        if (!err) {
            log('Sucess: User was alerted to a change in thier checks,via sms :', msg);
        } else {
            log('Erro could not send alert user who had a state change in their check', err);
        }
    });
};
workers.log = (originalCheckData, checkOutCome, state, alertWarranted, timeOfCheck) => {
    ...
    _logs.append(logFileName,logString,(err)=>{
        if(!err){
            log("Logging to file suceeded");
        }else{
            log(err);
            log("Logging to file failed");
        }
    });
    ...
    _logs.list(false, (err, logs) => {
        if (!err && logs && logs.length > 0) {
            logs.forEach((logName) => {
                // Compress data to a diferent file
                ...
                _logs.compress(logId, newFileId, (err) => {
                    if (!err) {
                        ...
                        _logs.truncate(logId, (err) => {
                            if (!err) {
                                log("Sucess truncating logFile");
                            } else {
                                log("Error truncating logFile");
                            }
                        })
                    }else{
                        log("Error compressing one of the log files", err)
                    }
                })
            })
        } else {
            log("Error : could not find any logs to rotate")
        }
    })
}
...
workers.init = () => {
    ...
    log('\x1b[33m%s\x1b[0m','Background workers are running')
