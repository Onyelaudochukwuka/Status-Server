...
handlers._users.POST = (data, callback) => {
  let key = typeof (data.headers.publickey) == 'string' ? data.headers.publickey : false;
  if (key) {
...
handlers._tokens.POST = (data, callback) => {
  ...
  handlers._ping.verifyKey()
if( != "m28yk6ju7nmqmevz780e") return callback(401, {'Error':'Missing publickey in header or publickey is invalid'});
  var phone = typeof (data.payload.phone) == 'string' && data.payload.phone.trim().length == 10 ? data.payload.phone.trim() : false;
  var password = typeof (data.payload.password) == 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : false;
  if (phone && password) {
...
handlers._ping.GET = (data, callback) => {
      //Hash the sent password, and comapre it to thw password stored in he object
        ...
        _data.create('publickeys', publickey, keyObject, (err) => {
          if (!err) {
            _data.read('publickeys',publickey,(err,userData)=>{
              if(!err && userData){
                callback(200,keyObject);
              } else {
            ...
            callback(500, {
              'Error': 'Could not create the new publickey'
            });
          }
        });
...
//Verify if a given token id is currently valid for a given user
handlers._ping.verifyKey = (key , callback) => {

  // lookup the token
  _data.read('publickeys', key, (err, tokenData) => {
    if (!err && tokenData) {
        callback(true);
    } else {
      callback(false);
